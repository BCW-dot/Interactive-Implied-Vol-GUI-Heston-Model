...
int main() {
    Kokkos::initialize();
    {
        ...
        // Surface dimensions
        const int width = 40;  // Number of strikes
        const int height = 20; // Number of maturities

        // Actual data ranges for visualization
        float min_strike = S_0 * 0.6f; //50% of spot
        float max_strike = S_0 * 1.2f; //130% of spot
        float min_maturity = 1.0f; //3 month maturity
        float max_maturity = 4.5f;//0.25f + ((height - 1) * 0.25f) * 0.5; //3 monnths + 20/4 months
        ...
        */
        ...
        std::vector<double> dividend_amounts = {0.70, 0.70, 0.70, 0.70};  //cash dividends
        std::vector<double> dividend_percentages = {0.0005, 0.0005, 0.0005, 0.0005};  // percentage dividends
        ...
        */
        ...
        // Call one of the two:
        //interactive_european(theta, m1, m2, total_size, width, height, min_strike, max_strike, min_maturity, max_maturity, S_0, v0, kappa, eta, sigma, rho, r_d, q);

        //interactive_american(theta, m1, m2, total_size, width, height, min_strike, max_strike, min_maturity, max_maturity, S_0, v0, kappa, eta, sigma, rho, r_d, q, dividend_dates, dividend_amounts, dividend_percentages);
        

        //Calibration
        interactive_calibration_european(theta, m1, m2, total_size, width, height, min_strike, max_strike, min_maturity, max_maturity, S_0, v0, kappa, eta, sigma, rho, r_d, q);
    }
    Kokkos::finalize();
    return 0;
}
